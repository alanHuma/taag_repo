...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    //GET DATA PETITION AXIOS
    loadDataEdit: function loadDataEdit() {
      var _this = this;

      axios__WEBPACK_IMPORTED_MODULE_1___default().get('../api/books/edit/' + this.id).then(function (response) {
        console.log(response.data[0]);
        _this.formData.id = response.data[0].id;
        _this.formData.author = response.data[0].author;
        _this.formData.title = response.data[0].title;
        _this.formData.edition = response.data[0].edition;
        _this.formData.publication_data = response.data[0].publication_data;
        _this.formData.content_type = response.data[0].content_type;
        _this.formData.restriction = response.data[0].restriction;
        _this.formData.matter = response.data[0].matter;
        _this.formData.provider = response.data[0].provider;
      });
    },
    //SEND DATA PETITION AXIOS
    submitForm: function submitForm() {
      axios__WEBPACK_IMPORTED_MODULE_1___default().post('../api/books/update/' + this.id, this.formData).then(function (res) {
